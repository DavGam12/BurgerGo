...
create table orders(order_id number(10) primary key, order_state varchar2(40) not null, direction varchar2(100) not null, order_price number(6,2) not null, order_date varchar2(12) not null, employee_id number(3), foreign key(employee_id) references employees(employee_id), customer_id number(8,0), foreign key(customer_id) references customers(customer_id));
